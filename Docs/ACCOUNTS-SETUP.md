...
- The authenticated dashboard now confirms 12-character passwords, six-digit email codes expiring after 600 seconds, secure password/email changes, TOTP enabled and the exact native callback allowed. The account-deletion function is deployed with gateway JWT verification enabled. See `AUTH-CHANGES-READY.md` for verified settings and test limitations.
- Custom SMTP is off, so customer email delivery and branded OTP templates remain blocked. No user was created/deleted and no confirmation email was sent. Live account, recovery, MFA and deletion tests remain outstanding.
...
The next setup step is to configure the owner's actual sending domain/provider through custom SMTP, apply the prepared code templates and provide real website/privacy/terms URLs. Complete live acceptance checks before enabling accounts in a new IPA. The app's public key cannot perform project administration.
...
3. Change the Confirm signup and Reset password email templates to show `{{ .Token }}` as a six-digit code. New Free projects created from 3 June 2026 require custom SMTP before these templates can be changed; see [Supabase's change notice](https://supabase.com/changelog/46599-changes-to-email-template-customisation-on-free-tier). This client uses in-app OTP verification, not email links. The prepared templates/settings in `AUTH-CHANGES-READY.md` use six-digit codes and a 10-minute expiry. Set the site URL to your real website. Allow exactly `riseandbake://auth/callback` for the native Google OAuth callback, without wildcards.
...
5. Deploy `supabase/functions/delete-account/index.ts` using the Supabase CLI or dashboard. It uses the platform's server-side service-role environment; never copy that key into the app. Keep gateway JWT verification enabled; current Supabase documentation supports legacy and asymmetric user JWTs. The function independently calls `auth.getUser(token)` regardless. Test that anonymous/stale/AAL1 deletion attempts fail.
